...
class CardBrowseJDBCDao
    ...
    constructor(
        ...
        companion object {
            ...
            private const val UNIQUE_MONSTER_TYPES =
            ...
            private const val UNIQUE_MONSTER_SUB_TYPES =
            ...
            private const val UNIQUE_LEVEL_VALUES_QUERY =
                "SELECT DISTINCT CAST(JSON_EXTRACT(monster_association, '$.level') AS DOUBLE) AS level FROM cards WHERE monster_association LIKE '%level%' ORDER BY level"
            private const val UNIQUE_RANK_VALUES_QUERY =
                "SELECT DISTINCT CAST(JSON_EXTRACT(monster_association, '$.rank') AS DOUBLE) AS `rank` FROM cards WHERE monster_association LIKE '%rank%' ORDER BY `rank`;"
            private const val UNIQUE_LINK_VALUES_QUERY =
                "SELECT DISTINCT CAST(JSON_EXTRACT(monster_association, '$.linkRating') AS DOUBLE) AS linkRating FROM cards WHERE monster_association LIKE '%linkRating%' ORDER BY linkRating"
        ...
        override fun getBrowseResults(
            ...
            val monsterTypeCriteria =
                if (criteria.monsterTypes.isEmpty()) {
                    ".*"
                } else {
                    "^" +
                        java.lang.String
                            .join("|", criteria.monsterTypes)
                            .replace("?", "\\?")
                }
            val monsterSubTypeCriteria =
                if (criteria.monsterSubTypes.isEmpty()) {
                    ".*"
                } else {
                    ".+/" +
                        java.lang.String
     ...
     */
            ...
            val results =
                jdbcNamedTemplate.query(sql, sqlParams) { row: ResultSet, _: Int ->
                    Card(
                        ...
                        this.monsterAssociation =
                            MonsterAssociation.parseDBString(
                                row.getString(BrowseQueryDefinition.MONSTER_ASSOCIATION.toString()),
                                jsonMapper,
                            )
                    }
                }
        ...
        override fun getCardColors(): Set<String> {
            ...
            val cardColors: Set<String> =
                LinkedHashSet(jdbcNamedTemplate.query(sql) { row: ResultSet, _: Int -> row.getString(1) })
            stopWatch.stop()
