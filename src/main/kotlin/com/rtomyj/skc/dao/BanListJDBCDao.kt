...
WHERE duel_format = :format AND ban_list_date < :currentBanList""",
            ...
            val newCardList =
                jdbcNamedTemplate.query<List<CardsPreviousBanListStatus>>(query, sqlParams) { row: ResultSet ->
                    ...
                    while (row.next()) {
                        ...
                        val cardsPreviousBanListStatus =
                            CardsPreviousBanListStatus(
                                Card(
                                    row.getString(5),
                                    row.getString(1),
        ...
        override fun getBanListDates(format: String): BanListDates = throw UnsupportedOperationException(UNSUPPORTED_OPERATION_MESSAGE)

        override fun getBanListDetailsForCard(
            cardId: String,
            format: BanListFormat,
        ): List<CardBanListStatus> {
            val sqlParams = MapSqlParameterSource()
            sqlParams.addValue("cardId", cardId)
            ...
            return jdbcNamedTemplate.query(
                ...
                try {
                    return@query CardBanListStatus(
                        LocalDate.parse(row.getString(1), dbDateFormatter),
                        cardId,
                        row.getString(2),
                        format,
                    )
