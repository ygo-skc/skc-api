...
class RequestFilter : OncePerRequestFilter() {
	companion object {
		...
		fun configureMDC(httpServletRequest: HttpServletRequest, clientIP: String) {
			...
			MDC.put(CLIENT_IP_MDC, clientIP.replace("[", "").replace("]", ""))
			...
			MDC.put("userAgent", httpServletRequest.getHeader(org.springframework.http.HttpHeaders.USER_AGENT))
		}
	}
	...
	override fun doFilterInternal(request: HttpServletRequest, response: HttpServletResponse, chain: FilterChain) {
		try {
			val clientIP =
				if (StringUtils.hasLength(request.getHeader(X_FORWARDED_FOR))) request.getHeader(X_FORWARDED_FOR) else request.remoteHost
