	...
	override fun getRemovedContentOfBanList(banListDate: String): List<CardsPreviousBanListStatus> {
		...
		return jdbcNamedTemplate.query<List<CardsPreviousBanListStatus>>(query, sqlParams) { row: ResultSet ->
			...
			while (row.next()) {
				...
				removedCard.cardId = row.getString(1)
				removedCard.previousBanStatus = row.getString(2)
				removedCard.cardName = row.getString(3)
		...
		val newCardList =
			jdbcNamedTemplate.query<List<CardsPreviousBanListStatus>>(query, sqlParams) { row: ResultSet ->
				...
				while (row.next()) {
					...
					cardsPreviousBanListStatus.cardId = cardID
					cardsPreviousBanListStatus.previousBanStatus = previousStatus
					cardsPreviousBanListStatus.cardName = row.getString(2)
