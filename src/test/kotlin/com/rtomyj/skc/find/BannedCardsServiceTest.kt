...
class BannedCardsServiceTest {
  ...
  inner class HappyPath {
    ...
    private fun happyPath(saveBandwidth: Boolean, fetchAllInfo: Boolean) {
      ...
      if (fetchAllInfo) {
        Mockito
            .verify(banListDiffService, Mockito.times(1))
            .getNewContentForGivenBanList(
              eq(TestConstants.BAN_LIST_START_DATE),
              eq("TCG")
            )
        Mockito
            .verify(banListDiffService, Mockito.times(1))
            .getRemovedContentForGivenBanList(
              eq(TestConstants.BAN_LIST_START_DATE),
              eq("TCG")
            )
      } else {
        Mockito
            .verify(banListDiffService, Mockito.times(0))
            .getNewContentForGivenBanList(
              eq(TestConstants.BAN_LIST_START_DATE),
              eq("TCG")
            )
    ...
    private fun assertCommonValues(
      banListInstance: BanListInstance,
      forbidden: List<Card>,
      limited: List<Card>,
      semiLimited: List<Card>
    ) {
     ...
     */
    private fun assertSaveBandwidthValues(
      forbidden: List<Card>,
      limited: List<Card>,
      semiLimited: List<Card>,
    ...
    private fun assertFetchAllInfoValues(banListInstance: BanListInstance, fetchAllInfo: Boolean) {
      if (fetchAllInfo) {
        ...
        Assertions.assertEquals(
          ...
          banListInstance.newContent!!.newLimited[0].card.cardName
        )
        Assertions.assertEquals(
          TestConstants.D_MALICIOUS_NAME,
          banListInstance.newContent!!.newLimited[1].card.cardName
        )

        Assertions.assertNotNull(banListInstance.removedContent)
        Assertions.assertEquals(
          TestConstants.STRATOS_NAME,
          banListInstance.removedContent!!.removedCards[0].card.cardName
        )
        Assertions.assertEquals(
          TestConstants.A_HERO_LIVES_NAME,
          banListInstance.removedContent!!.removedCards[1].card.cardName
        )
        Assertions.assertEquals(
          TestConstants.D_MALICIOUS_NAME,
          banListInstance.removedContent!!.removedCards[2].card.cardName
        )
      } else {
  ...
  inner class UnhappyPath {
    ...
    private fun dbError_BanListNotInDB(isSaveBandwidth: Boolean) {
      ...
      Assertions.assertEquals(
        String.format(
          ErrConstants.BAN_LIST_NOT_FOUND_FOR_START_DATE,
          TestConstants.BAN_LIST_START_DATE
        ), ex.message
      )
