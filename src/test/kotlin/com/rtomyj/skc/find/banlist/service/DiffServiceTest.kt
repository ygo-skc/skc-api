	...
	inner class HappyPath {
		...
		fun testFetchingBanListNewContent_FromDB_Success() {
			...
			Assertions.assertEquals(
				"/card/${TestConstants.STRATOS_ID}?allInfo=true", newForbiddenCards[0].card.links.getLink("self").get().href
			)
			Assertions.assertEquals(
				"/card/${TestConstants.A_HERO_LIVES_ID}?allInfo=true", newLimitedCards[0].card.links.getLink("self").get().href
			)
			Assertions.assertEquals(
				"/card/${TestConstants.D_MALICIOUS_ID}?allInfo=true", newLimitedCards[1].card.links.getLink("self").get().href
			)
		...
		fun testFetchingBanListRemovedContent_FromDB_Success() {
			...
			Assertions.assertEquals(
				"/card/${TestConstants.A_HERO_LIVES_ID}?allInfo=true", removedCards[1].card.links.getLink("self").get().href
			)
			Assertions.assertEquals(
				"/card/${TestConstants.D_MALICIOUS_ID}?allInfo=true", removedCards[2].card.links.getLink("self").get().href
			)
