Feature: Fetch information about an individual card. Use Card endpoints of SKC API to fetch info about a card.
    ...
    Scenario Outline: Fetching all information about a monster card and checking if response has the necessary attributes and values (see other scenarios for more specific value checks).
    ...
    Scenario Outline: Fetching all information about a spell/trap card and checking if response has the necessary attributes and values (see other scenarios for more specific value checks).
        Given I want all info for card with ID "<cardID>" - IE, info on ban lists, products, etc
        When I request info for the card
        Then http status of response should be <httpStatus>
        And card info return value should contain "<cardName>", "<cardID>", "<cardColor>", "<cardAttribute>"
